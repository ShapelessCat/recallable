# Combined Project Critique: recallable

**Date:** 2026-03-22

**Evaluators:**

- Gemini 3.1 Pro High — Antigravity v1.0
- Codex GPT-5.4
- Claude Opus 4.6 (1M context)

This report consolidates findings from three independent reviews into a
single deduplicated document. Each finding is tagged with the model(s) that
raised it. Where reviewers disagreed on framing or priority, those differences
are noted.

---

## 1. Overall Assessment

> **Consensus (all three):** `recallable` is a well-engineered `0.1.0` crate
> with strong internal discipline — clean clippy, broad test matrix, coverage
> tracking, modular macro implementation — and a user-facing contract that
> does not yet match the quality of the code underneath it.

All three reviewers passed `cargo test` and `cargo clippy` without issues.
Codex additionally confirmed edge-case behaviors with offline throwaway
crates. No implementation bugs were found by any reviewer.

The unanimous verdict: ready for controlled internal use today, not yet ready
for broad public adoption until documentation, error messages, and implicit
requirements are tightened.

---

## 2. Architecture and Design

### 2.1. Clean Trait Hierarchy

**Raised by:** Gemini, Codex, Claude

The separation of `Recallable` (declares memento type), `Recall` (infallible
application), and `TryRecall` (fallible application with custom error) is
clean and idiomatic. The blanket `TryRecall` impl for all `Recall` types
mirrors the `From`/`TryFrom` pattern in the standard library and provides
excellent developer ergonomics.

### 2.2. Hidden Memento Struct

**Raised by:** Gemini, Claude

Generating the companion struct inside `const _: () = { ... }` and exposing
it only as `<Struct as Recallable>::Memento` prevents namespace pollution and
gives the macro freedom to change internal naming without breaking downstream
code.

### 2.3. `no_std` by Default

**Raised by:** Gemini, Claude

The traits crate is `#![no_std]`, which is the right default for a library of
this kind. Gemini specifically praised compatibility with embedded environments
and specialized runtimes.

### 2.4. Well-Structured Macro Internals

**Raised by:** Gemini, Codex, Claude

The `MacroContext` pattern, with separate modules for each code-generation
concern (`memento_struct.rs`, `recallable_impl.rs`, `recall_impl.rs`,
`from_impl.rs`, `utils.rs`), is clean and maintainable. Gemini noted the
proper use of `proc-macro-crate` for crate rename handling. Codex described
the implementation as "structured instead of being one large expansion
function."

### 2.5. Smart Serde Integration

**Raised by:** Gemini

Gemini viewed the serde integration positively: automatically deriving
`Deserialize` but not `Serialize` on mementos aligns with durable-execution
use cases where mementos are received (deserialized) but the original struct
is what gets serialized. Injecting `#[serde(skip)]` on `#[recallable(skip)]`
fields keeps serialization and state-restoration in sync.

**Counterpoint (Codex, Claude):** Both Codex and Claude viewed this same
behavior as problematic coupling — see Section 5.

### 2.6. No `TryRecall` Derive

**Raised by:** Claude only

`Recall` has a derive macro, but `TryRecall` does not. Users who need
validation logic during recall must implement the full trait manually. For a
crate that markets itself on reducing boilerplate, this is a notable
asymmetry.

### 2.7. No Partial-Recall Semantics

**Raised by:** Claude only

The `recall` method replaces all non-skipped fields unconditionally. There is
no way to express "apply only the fields that are present" (like serde's
`Option<T>` flattening pattern). This limits the crate's applicability to the
partial-update scenarios the README motivates.

### 2.8. Operational Fragility of Separate Derives

**Raised by:** Claude only

`Recall` requires `Recallable` to be implemented first (for `Self::Memento`).
If a user forgets to derive one, the resulting error does not clearly point to
the missing derive. `#[recallable_model]` papers over this, but direct derive
users are left to figure it out.

---

## 3. Generic and Lifetime Constraints

### 3.1. README Oversells Generic Support

**Raised by:** Codex, Claude (Gemini notes the limitation but does not flag
the README contradiction)

`README.md:60` claims "Full support for generic types." `README.md:239`
says `#[recallable]` only supports simple generic types. These two statements
directly contradict each other in the same document.

The actual restrictions:

- `T` (bare type parameter) — accepted
- `Option<T>`, `Vec<T>` — rejected
- `<T as Trait>::Assoc` — rejected
- `mod::ConcreteType` — accepted (no generic param tracking)
- Any lifetime parameter — rejected

Codex confirmed two of these limits empirically with offline throwaway crates.

**Recommendation (Codex, Claude):** Replace the feature claim with accurate
language like "Support for simple generic type parameters" and move the
Limitations section before the examples. Codex frames this as a
"product-positioning problem," not just a wording issue.

### 3.2. Lifetime Rejection Is Broader Than Documented

**Raised by:** Codex, Claude (Gemini notes it as a constraint)

The `validate_generics` function rejects any struct with *any* lifetime
parameter — not just structs with borrowed fields. The error message says
"Recall derives do not support borrowed fields," which is misleading. A
struct like `Foo<'a> { data: PhantomData<&'a ()> }` contains no borrowed
runtime data but is still rejected.

Codex confirmed this with a throwaway crate. All documentation
(`README.md:238`, `README.md:291-302`, the compiler error text) conflates
"borrowed fields" with "lifetime-parameterized structs."

**Recommendation (Codex, Claude):** Change the error message to "Recall
derives do not support lifetime-parameterized structs" and update the README
to match, unless the implementation is broadened to only reject actual borrow
types.

### 3.3. Enum and Union Support

**Raised by:** Gemini

State machines frequently use enums for state representation. The absence of
enum support restricts `recallable` to struct-only architectures. Gemini
recommends investigating enum memento derivation as a future feature.

---

## 4. Hardcoded Memento Derives

**Raised by:** Gemini, Codex, Claude

The generated memento struct always derives `Clone`, `Debug`, `PartialEq`
(and `Deserialize` when the `serde` feature is enabled). This creates two
problems:

1. **Hidden requirements.** If any field type does not implement all three
   traits, the derive expansion fails with an error pointing at generated
   code, not the user's struct. These requirements are not documented
   anywhere in user-facing materials. Codex calls this "a bad kind of magic"
   — the public API looks minimal (`trait Recallable { type Memento; }`) but
   the derive macro imposes a much larger undocumented contract.

2. **No customization.** Users who need `Eq`, `Hash`, `Default`, or `Ord`
   on the memento cannot get them without manually implementing `Recallable`
   and defining the memento struct themselves, defeating the macro's purpose.

**Recommendation (all three):** Add a `#[recallable(memento_derive(...))]` or
`#[recallable(memento_attr(...))]` attribute to let users control the derived
traits. At minimum, document the implicit `Clone`/`Debug`/`PartialEq`
requirements prominently.

---

## 5. `#[recallable_model]` and Serde Coupling

### 5.1. Invisible Attribute Mutation

**Raised by:** Codex, Claude (Gemini views this positively)

When the `serde` feature is enabled, `#[recallable_model]`:

1. Adds `#[derive(serde::Serialize)]` to the struct.
2. Injects `#[serde(skip)]` onto fields marked `#[recallable(skip)]`.
3. Rejects any pre-existing `#[derive(Serialize)]` with a compile error.

Codex's framing: "For a macro library, invisible attribute mutation should be
treated as expensive API surface, not free convenience." Users lose control
over whether serialization derives are explicit or implicit, the macro
becomes harder to reason about, and the crate couples "state recalling model"
with "this exact serde behavior" more tightly than necessary.

Claude adds the attribute-ordering footgun: `#[recallable_model]` must appear
*before* other attributes in source order (because attribute macros only see
attributes listed after them). This requirement is documented only in internal
project files (`CLAUDE.md`), not in user-facing docs.

**Recommendation (Codex, Claude):** Make `#[recallable_model]` less magical
or document every injected attribute prominently. Document the
attribute-ordering requirement in the README.

### 5.2. Serde Attribute Forwarding Risk

**Raised by:** Claude only

The memento's field layout must be wire-compatible with the original struct's
serialized form. If a user adds `#[serde(rename = "...")]` or
`#[serde(flatten)]` to the original struct, those attributes are not
forwarded to the invisible memento, creating a silent deserialization mismatch
at runtime.

**Recommendation:** Either document this structural coupling explicitly, or
provide a way to forward serde attributes from original struct fields to
their corresponding memento fields.

### 5.3. Asymmetric Serialization Design

**Raised by:** Gemini (positive), Claude (cautious)

The generated memento derives `Deserialize` but not `Serialize`. Gemini views
this as a smart design choice aligned with durable-execution use cases. Claude
agrees it is intentional but flags the structural coupling risk described
above.

---

## 6. Documentation and Onboarding

### 6.1. README Wording Issues

**Raised by:** Codex, Claude

- `README.md:52-53` contains a garbled sentence: "More or creating a derive
  attribute, inserting `Recallable` and `Recall`..." This looks like an
  unfinished edit.
- The "Full support for generic types" claim at line 60 directly contradicts
  the limitations at line 239 (see Section 3.1).

### 6.2. Installation and Examples Mismatch

**Raised by:** Codex, Claude

The installation section tells users to add only `recallable`, but the first
example immediately uses `serde`, `postcard`, and `heapless` without
mentioning they need to be added separately. Codex frames this as a
"first impressions" problem — users copy examples before they read
limitations.

### 6.3. Limitations Section Placement

**Raised by:** Codex, Claude

The Limitations section is near the bottom of the README, after all examples.
Users will have already copy-pasted code and hit confusing errors before
discovering constraints. Codex recommends a dedicated "limitations and
non-goals" section near the top.

### 6.4. Missing Examples Directory

**Raised by:** Gemini, Codex, Claude

There is no `examples/` directory. `CONTRIBUTING.md:33-39` contains a
commented-out "Running Examples" section with a TODO. For a proc-macro crate,
standalone runnable examples are one of the most effective onboarding tools.

### 6.5. Thin Changelog

**Raised by:** Codex, Claude

The changelog contains a single bootstrap entry that does not describe the
initial feature set in enough detail to be useful for evaluation.

### 6.6. Memento Type Alias Ergonomics

**Raised by:** Gemini

The `<Struct as Recallable>::Memento` syntax is verbose. Providing
documentation hints or examples showing how to create type aliases
(e.g., `type MyMemento = <MyStruct as Recallable>::Memento;`) would reduce
friction.

### 6.7. Attribute Ordering Documentation

**Raised by:** Gemini, Claude

`#[recallable_model]` must appear before other attributes it needs to inspect.
Gemini noted this is documented in `CLAUDE.md` but recommended a prominent
README note. Claude confirmed it is not in user-facing docs.

---

## 7. Testing

### 7.1. Test Breadth

**Raised by:** Gemini, Codex, Claude (all positive)

The test suite covers `serde_json`, `postcard`, `no_serde`, `impl_from`,
named structs, tuple structs, unit structs, generic types, nested recallable
fields, skipped fields, where clauses, and compile-fail UI tests via
`trybuild`. Codex notes the separate feature-gated test binaries with
explicit `required-features` in `Cargo.toml`. All three consider this
impressive for a `0.1.0` crate.

### 7.2. CI Pipeline

**Raised by:** Gemini, Codex, Claude (all positive)

CI checks stable and MSRV (Rust 1.88.0), runs multiple feature combinations,
and includes a coverage comparison job with thresholds (function >= 100%,
line >= 90%, region >= 90%). Codex calls this "stronger than expected for a
`0.1.0` crate."

**Minor concern (Claude):** The coverage thresholds are implemented entirely
in shell script within the CI YAML, making them hard to maintain and test
locally.

### 7.3. Missing Runtime Edge-Case Tests

**Raised by:** Claude only

No tests cover runtime edge cases like deserialization from malformed data or
field reordering between serialization and deserialization. These are the
scenarios most likely to bite users of a persistence-focused library.

### 7.4. Minimal Doc Tests

**Raised by:** Claude only

Only two doc tests exist (on `Recallable` and `TryRecall`). The `Recall`
trait, `recallable_model` macro, and field-level attributes have no doc tests.

### 7.5. No Property-Based or Fuzz Testing

**Raised by:** Claude only

For a crate operating at serialization boundaries, property-based tests
(e.g., `proptest` or `quickcheck`) verifying round-trip invariants would
add significant confidence.

---

## 8. Project Hygiene

### 8.1. Editor Configuration Committed

**Raised by:** Codex, Claude

`.vscode/settings.json` is tracked in git (containing only file associations
for license files). The `.gitignore` excludes `.idea/` but not `.vscode/`,
which is inconsistent.

**Recommendation:** Either commit both editor configs intentionally or ignore
both.

### 8.2. Dependabot Scope

**Raised by:** Claude only

The `dependabot.yaml` covers `github-actions` updates but not Cargo
dependencies. For a crate with pinned `proc-macro2`, `syn`, `quote`, and
`proc-macro-crate` versions, automated dependency updates would be valuable.

---

## 9. Code-Level Observations

### 9.1. `HashMap` Non-Deterministic Iteration

**Raised by:** Claude only

`MacroContext` uses `HashMap<&Ident, TypeUsage>` for `preserved_types`.
`HashMap` iteration order is non-deterministic. Currently safe because
`build_memento_struct_type` iterates `generics.type_params()` in declaration
order and only checks membership, but fragile if the iteration pattern ever
changes.

### 9.2. `SimpleTypeCollector` Over-Collects

**Raised by:** Claude only

The `collect_used_simple_types` visitor walks the entire type tree and
collects the first segment of every `TypePath`. For a type like
`std::collections::HashMap<K, V>`,
it collects `std`, `K`, and `V`. The `std` entry is harmless but the
over-collection is semantically sloppy.

### 9.3. `#[inline(always)]` on Generated Methods

**Raised by:** Claude only

Both the generated `recall` method and the blanket `TryRecall::try_recall`
are marked `#[inline(always)]`. For structs with many fields, forced inlining
could cause code bloat. `#[inline]` (without `always`) would let the compiler
decide.

### 9.4. Proper Trait Bound Inference

**Raised by:** Gemini

The automatic inference of trait bounds for `Recallable` is properly executed,
avoiding the over-constraining issues typical in many standard library
derives. Generic parameters used only by skipped fields are correctly pruned
from the memento type.

### 9.5. `proc-macro-crate` Usage

**Raised by:** Gemini

`proc-macro-crate` is sensibly used to resolve the actual crate name from
`Cargo.toml`, handling crate renames correctly.

---

## 10. Prioritized Recommendations

### High Priority

1. Fix README: replace "Full support for generic types" with
   accurate language.
   *— Codex, Claude*
2. Document implicit `Clone`/`Debug`/`PartialEq` requirements
   on mementos.
   *— Gemini, Codex, Claude*
3. Fix lifetime error message: "lifetime-parameterized structs"
   not "borrowed fields."
   *— Codex, Claude*
4. Fix garbled sentence at README line 52-53.
   *— Codex, Claude*
5. Document or reduce `#[recallable_model]` invisible serde
   behavior.
   *— Codex, Claude*
6. Move Limitations section before examples in README.
   *— Codex, Claude*

### Medium Priority

1. Add `#[recallable(memento_derive(...))]` for user-controlled
   memento derives.
   *— Gemini, Codex, Claude*
2. Create `examples/` directory with runnable examples.
   *— Gemini, Codex, Claude*
3. Document `#[recallable_model]` attribute-ordering requirement
   in README.
   *— Gemini, Claude*
4. Fix installation section to mention `serde`/`postcard`
   dev-dependencies.
   *— Codex, Claude*
5. Stop tracking `.vscode/settings.json` or add `.vscode/` to
   `.gitignore`.
   *— Codex, Claude*
6. Enhance generic container support (`Option<T>`, `Vec<T>`).
   *— Gemini, Claude*
7. Investigate enum support.
   *— Gemini*

### Low Priority

1. Consider `#[inline]` instead of `#[inline(always)]` for
   generated methods.
   *— Claude*
2. Add property-based round-trip tests for serialization
   invariants.
   *— Claude*
3. Expand doc tests to cover `Recall`, `recallable_model`, and
   field attributes.
   *— Claude*
4. Consider supporting `Option<T>` for partial-update semantics.
   *— Claude*
5. Expand changelog beyond bootstrap entry.
   *— Codex, Claude*
6. Add `cargo` ecosystem to dependabot configuration.
   *— Claude*
7. Document memento type-alias pattern to reduce verbosity of
   `<T as Recallable>::Memento`.
   *— Gemini*
8. Extract CI coverage logic from inline shell into a
   maintainable script.
   *— Claude*

---

## 11. Summary

### What All Three Agree On

1. The project is well-engineered internally — clean code, modular macro
   structure, strong CI, broad test coverage, no bugs found.
2. Hardcoded memento derives create hidden requirements and prevent
   customization.
3. Generic support is narrower than the README claims.
4. Lifetime error messaging conflates "borrowed fields" with
   "lifetime-parameterized structs."
5. There is no `examples/` directory.
6. The crate is promising but not yet ready for broad public adoption.

### Where Reviewers Diverge

- **Serde coupling:**
  Gemini views it positively (smart design).
  Codex views it negatively (too opinionated).
  Claude is cautious (structural risk).
- **Top priority:**
  Gemini says feature expansion.
  Codex and Claude say documentation precision.
- **Enum support:**
  Recommended by Gemini only.
- **Partial recall:**
  Flagged as missing by Claude only.
- **`TryRecall` derive:**
  Flagged as a gap by Claude only.
- **Code-level issues:**
  3 issues found by Claude only.
- **Empirical confirmation:**
  Gemini used code reading only.
  Codex used offline throwaway crates.
  Claude ran full test suite + clippy.

### The Core Tension

Gemini argues the crate's primary bottleneck is **missing capability** —
broader generic support, enum support, customizable derives. Codex and Claude
argue the primary bottleneck is **imprecise promises** — the README, error
messages, and implicit requirements do not accurately represent what the
macros can and cannot do.

These are not contradictory. The recommended path is:

1. **First:** Tighten the user contract — fix documentation, error messages,
   and implicit requirements so users can trust what the crate says it does.
2. **Then:** Expand capability — broader generics, enums, customizable
   derives — on top of a solid, honest foundation.

Expanding capability before clarifying behavior would make the crate more
powerful but not more trustworthy.
